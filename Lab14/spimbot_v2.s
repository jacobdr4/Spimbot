...
.text
main:
    sub     $sp, $sp, 36                        # allocate stack/save s-registers
    sw      $ra, 0($sp)
    sw      $s0, 4($sp)
    sw      $s1, 8($sp)
    sw      $s2, 12($sp)
...
# head toward tile [11,12] = pixel [88,96] to start collecting stone/wood
    li      $a0, 88
    li      $a1, 96
...
initial_move:
    ...
    bge     $t0, $t1, stop_1                    # if y < 96, keep moving
    j       initial_move
...
collect_1:
    ...
    # IS THERE A WAY TO CHECK IF TILE IS STILL A STONE/WOOD (?)
    # DOES GET_MAP UPDATE WHEN BLOCKS BROKEN?
    
    # STONE (CHOOSE 8)
    # located at tiles [10-13,11] and [14,12] and [9-13,13]
    li      $t0, 0x0A0B                         # tile [10,11]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0B0B                         # tile [11,11]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0C0B                         # tile [12,11]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0D0B                         # tile [13,11]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0E0C                         # tile [14,12]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x090D                         # tile [9,13]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0A0D                         # tile [10,13]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0B0D                         # tile [11,13]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0C0D                         # tile [12,13]
    sw      $t0, BREAK_BLOCK


    # WOOD (CHOOSE 8)
    # located at tiles [8-13,12] and [10-11,10]
    li      $t0, 0x080C                         # tile [8,12]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x090C                         # tile [9,12]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0A0C                         # tile [10,12]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0B0C                         # tile [11,12]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0C0C                         # tile [12,12]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0D0C                         # tile [13,12]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0A0A                         # tile [10,10]
    sw      $t0, BREAK_BLOCK

    li      $t0, 0x0B0A                         # tile [11,10]
    sw      $t0, BREAK_BLOCK
...
move_in:
    sw      $s3, ANGLE                        
    sw      $s1, ANGLE_CONTROL                  # absolute
    sw      $s2, VELOCITY                       # max velocity

# tile [15,16]
promised_land_1:
    lw      $t0, BOT_Y                          # $t0 is y-coord
    li      $t1, 132                            # (pixel 132 -> mid tile 16)
    bge     $t0, $t1, go_east
    j       promised_land_1
...
promised_land_2:
    ...
    li      $t1, 124                            # (pixel 124 -> mid tile 15)
    bge     $t0, $t1, slow_up
    j       promised_land_2

slow_up:
    sw      $zero, VELOCITY                     # STOP (at tile [15,16])

    # BUILD FIRST SET OF STONE WALLS, DOOR
    ...
    # should still have 1 stone and 2 wood?

    # DRINK
    li      $t0, 0x1010                         # tile [16,16] (water)
    sw      $t0, USE_BLOCK
...
craft_2:
    ...
    # LEFT WALL 1
    li      $t0, 0x00040E10                     # stone wall at [14,16]
    sw      $t0, PLACE_BLOCK

    li      $t0, ID_STONEWALL
    sw      $t0, CRAFT                          # stone wall #4

    # BOTTOM WALL 1
    li      $t0, 0x00040F11                     # stone wall at [15,17]
    sw      $t0, PLACE_BLOCK

    li      $t0, ID_STONEWALL
    sw      $t0, CRAFT                          # stone wall #5

    # RIGHT WALL 1
    li      $t0, 0x00041010                     # [16,16]
    sw      $t0, PLACE_BLOCK

    li      $t0, ID_STONEWALL
    sw      $t0, CRAFT                          # stone wall #6

    # UPPER DOOR
    li      $t0, 0x00070F0F                     # Door at [15,15]
    sw      $t0, PLACE_BLOCK
    ...
    # BOTTOM WALL 2
    li      $t0, 0x00040F12                     # stone wall at [15,18]
    sw      $t0, PLACE_BLOCK

    # RIGHT WALL 2
    li      $t0, 0x00041110                     # [17,16]
    sw      $t0, PLACE_BLOCK
    ...
    # j       slow_up       # only when MORNING again! (timer interrupt?)
...
bonk_interrupt:
    ...
    # how can I know when to use ATTACK?
...
request_puzzle_interrupt:
    ...
    # I seem to be able to craft multiple items w/ just 1 creativity?
...
respawn_interrupt:
    ...
    # head back to fort location, tile [14,16]
    #li      $a0, 112
    #li      $a1, 128
...
night_interrupt:
    ...
    # WHILE NIGHTFALL:
    # how can I repeat this until day?
    #li      $t0, 0x0C10                       # tile [12,16] (outside left wall)
    #sw      $t0, ATTACK

    #li      $t0, 0x0E12                       # tile [14,18] (outside door)
    #sw      $t0, ATTACK

    #li      $t0, 0x0E0E                       # tile [14,14] (outside upper wall)
    #sw      $t0, ATTACK

    #li      $t0, 0x1010                       # tile [16,16] (outside right wall)
    #sw      $t0, ATTACK
